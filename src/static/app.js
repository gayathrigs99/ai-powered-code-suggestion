      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (participants.length > 0) {
          ...
          participants.forEach((p) => {
            // include data attributes for activity name and email so handlers can reference them
            participantsHtml += `    <li><span class="participant-email">${p}</span><button class="remove-btn" data-activity="${name}" data-email="${p}" aria-label="Remove participant">✕</button></li>\n`;
          });
          participantsHtml += '  </ul>\n';
        } else {
